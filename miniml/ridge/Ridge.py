...
class Ridge:
    ...
    def algebraicSol(self):
        pseudo = np.linalg.inv(np.matmul(self.x.T, self.x) + self.N*self.lambd)
        self.w = np.matmul(np.matmul(pseudo, self.x.T), self.y)
        
    def J_n(self, n):
        return (2/self.N)*(np.matmul(self.w, self.x[n])-self.y[n])*self.x[n] + 2*self.lambd*self.w
    ...
    def fit(self, max_epoch=100, sol='algebra', kernel='linear', lambd=1, lr=0.01, d=3):
        self.sol = sol
        self.lambd = lambd
        ...
        return [max_epoch, self.sol, self.lambd, self.lr]
